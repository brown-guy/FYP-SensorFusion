...
def extended_kalman_filter(depth_measured, depth_ground_truth,
    ...
    # Define the function that will be mapped to the depth maps
    def ekf_pixel(pixel):
        i, j = pixel
    ...
    # Use the map function to apply the EKF to each pixel in the depth maps
    pixels = [(i, j) for i in range(depth_measured.shape[0]) for j in range(depth_measured.shape[1])]
    map(ekf_pixel, pixels)
...
observation_covariance = np.ones((1, 1)) * 1  # Covariance matrix for the measurement model noise

# Generate some example depth maps
for a in range(5,99):
    depth_measured = rd.depth_read(rf"D:\MTS\Datasets\2011_09_26_drive_0009_sync\image_02\depth maps generated\{a}_compressed.png")
    depth_ground_truth = rd.depth_read(rf"D:\MTS\Datasets\2011_09_26_drive_0009_sync\image_02\ground truth Lidar\{a}_compressed.png")
   ...
   # print(depth_estimate)  # Output: estimated depth map of size 375x1242
    ...
    cv2.imwrite(rf"D:\MTS\Datasets\2011_09_26_drive_0009_sync\image_02\Fused depth maps\{a}.png", depth_uint16)
